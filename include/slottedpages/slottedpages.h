...
const uint16_t SLOT_SIZE = sizeof(Slot); // the size of slot
...
public:
    void remove(uint16_t slot_id); // remove the slot with slot_id

    uint16_t store(uint32_t len, const char *data);  // stores a new data in the page

    void store(uint32_t len, const char *data, uint16_t slot_id); // changes the data on the slot

    bool is_free(int len); // check page contains data with the length of len

    void redirect(uint16_t slot_id, TID redirect_tid); // putting tid on the slot data, which means it is quite big

    uint16_t get_length(uint16_t slot_id); // the length of the data on the slot

    uint16_t get_offset(uint16_t slot_id); // the offset of the data on the slot

    char* get_data(uint16_t slot_id); // return the data on the slot
